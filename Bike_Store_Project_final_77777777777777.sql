...
EXEC sp_MagazinKPI_hisobla @StoreID = 1;
...
--###############################################################
--1. KPI Magazinlar Savdosining Ortacha summasi 
select 
sum(MagazinOrtachaSavdosi) as Magazin_Savdosi_Ortacha_Summalari
from dbo.vw_MagazinSavdosiOrtachaSummalari
--2-KPI Kategoriyalardan kelib chiqqan holda umumiy foyda

select sum(Foydasi) as Umumiy_foyda
from [dbo].[vw_MahsulotKategoriyasiBoyichaSotuvlar]

--3-KPI har mahsulot boyicha ortacha narxi 
 
SELECT AVG(JamiSavdo) AS AvgSalePerProduct
FROM [dbo].[vw_MahsulotSavdosiReytingi];

--4-KPI eng ko'p sotilgan mahsulot summasi 
SELECT TOP 1 JamiSavdo as Top_1_Mahsulot_summasi
FROM [dbo].[vw_MahsulotSavdosiReytingi]
ORDER BY JamiSavdo DESC;

--5-KPI  TOP Mintaqa boyicha 
select * from [dbo].[vw_MintaqaviyTrendlar]
select TOP 1
magazin_davlat as Top_1_Mintaqa
from 
[dbo].[vw_MintaqaviyTrendlar] 
order by Foydasi DESC

--6-KPI Top bir mintaqa foydasi
select TOP 1 Foydasi as Top_1_mintaqa_foydasi
from[dbo].[vw_MintaqaviyTrendlar]
order by  Foydasi DESC

--7-KPI Ombordagi eng kam mahsulot 
select * from 
[dbo].[vw_OmbordagiMahsulotlar]

select TOP 1 mahsuot_nomi as  Ombordagi_eng_kam_mahsulot
from [dbo].[vw_OmbordagiMahsulotlar]
order by ombordagi_soni ASC

--8-KPI Ombordagi eng kam mahsulot soni 
select Top 1 ombordagi_soni as Eng_kam_mahsulot_soni
from [dbo].[vw_OmbordagiMahsulotlar]
order by ombordagi_soni ASC

--9-KPI  
select  *from [dbo].[vw_XodimlarSamaradorligi]

select Top 1 xodim_ismi as Top_1_xodim
from [dbo].[vw_XodimlarSamaradorligi]
order by Olingan_foydasi DESC

--10-KPI Top 1 xodim keltirgan foydasi  
select Top 1 Olingan_foydasi as Top_1_xodim_foydasi
from [dbo].[vw_XodimlarSamaradorligi]
order by Olingan_foydasi DESC
--###########################################################
 
--Agent uchun Daily_report table yarataman:

CREATE TABLE dbo.Daily_Report (
    ReportDate DATE NOT NULL PRIMARY KEY,  -- bugungi sana
    Magazin_Savdosi_Ortacha_Summalari DECIMAL(18,2) NULL,
	Umumiyfoyda DECIMAL(18,2) NULL,
	AvgSalePerProduct DECIMAL(18,2) NULL,
	Top_1_Mahsulot_summasi DECIMAL(18,2) NULL,
	Top_1_Mintaqa VARCHAR(100),
	Top_1_mintaqa_foydasi DECIMAL(18,2) NULL,
	Ombordagi_eng_kam_mahsulot VARCHAR(100),
	Eng_kam_mahsulot_soni DECIMAL(18,2) NULL,
	Top_1_xodim VARCHAR(100),
	Top_1_xodim_foydasi DECIMAL(18,2) NULL
);
